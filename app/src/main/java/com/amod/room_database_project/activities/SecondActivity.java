package com.amod.room_database_project.activities;
...
import com.amod.room_database_project.database.AppExecutors;
...
public class SecondActivity extends AppCompatActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        intent = getIntent();
        if (intent != null && intent.hasExtra(Constants.Person_Id)) {
            button.setText("Update");

            mPersonId = intent.getIntExtra(Constants.Person_Id, -1);
    ...
    public void onSaveButtonClicked() {
        ...
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            @Override
            public void run() {
                if (!intent.hasExtra(Constants.Person_Id)) {
